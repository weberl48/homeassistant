"""RSI(2) + ATR-from-EMA mean-reversion on liquid crypto pairs.

Same statistical edge as src/strategies/mean_reversion.py, retuned for crypto's
faster cadence: hourly bars, looser RSI/stretch thresholds, wider stop, shorter
max-hold.
...
WATCHLIST: list[str] = ["BTC/USD", "ETH/USD"]
POSITION_SIZE_USD: float = 22.50      # matches risk.PER_ORDER_CAP_USD[CRYPTO_MEAN_REVERSION]
ENTRY_RSI_THRESHOLD: float = 15.0     # looser than ETF's 10 — hourly bars are noisier
ENTRY_STRETCH_ATR_MULT: float = 1.0   # looser than ETF's 1.5 — crypto stretches less consistently
EXIT_STOP_LOSS_PCT: float = 0.05      # wider than ETF's 3% — crypto volatility
EXIT_MAX_HOLD_HOURS: int = 24         # crypto mean-reverts faster than ETFs
RSI_PERIOD: int = 2
EMA_PERIOD: int = 20
ATR_PERIOD: int = 14
LIMIT_PREMIUM_OVER_ASK: float = 0.005  # 0.5% — within risk.LIMIT_PRICE_MAX_PREMIUM (1.0% for crypto)
HOURLY_LOOKBACK: int = 240             # ~10 days of hourly bars
...
def compute_entries(client: "AlpacaClient", excluded_symbols: set[str]) -> list[EntrySignal]:
    ...
    for symbol in WATCHLIST:
        if symbol in excluded_symbols:
            continue
        bars = client.crypto_hourly_bars(symbol, hours=HOURLY_LOOKBACK)
        ...
        if cond_rsi and cond_stretch:
            ask = client.latest_crypto_ask(symbol)
...
def compute_exits(client: "AlpacaClient", open_trades: list["OpenTrade"]) -> list[ExitSignal]:
    """For each crypto open position: exit on EMA touch / -5% / >24h held."""
    exits = []
    for ot in open_trades:
        if ot.strategy != "crypto_mean_reversion":
            continue
        if ot.filled_avg_price is None:
            continue
        bars = client.crypto_hourly_bars(ot.symbol, hours=72)
        if not bars:
            continue
        closes = np.array([b.c for b in bars], dtype=np.float64)
        last = float(closes[-1])
        ...
        hours_held = (datetime.utcnow() - ot.opened_at).total_seconds() / 3600.0
        ...
        elif hours_held >= EXIT_MAX_HOLD_HOURS:
            exits.append(ExitSignal(ot.symbol, True, f"max_hold: held {hours_held:.1f}h", last))
        else:
            exits.append(ExitSignal(
                ot.symbol, False,
                f"hold: pnl={pnl_pct*100:.2f}% ema_dist={(ema-last)/last*100:.2f}% h={hours_held:.1f}",
                last,
            ))
    return exits
